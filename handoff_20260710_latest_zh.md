...
09:22 低同步早退 A/B 仍不进默认。`ANALOG_RETRY_ON_LOW_SYNC=1`、`ANALOG_LOW_SYNC_RETRY_THRESHOLD=0.06` 的 50 张 `batch-1783732885-50` 为 `50/50`、fallback `0`，TVM p95 `243.78 ms`，IQ p95/max `273.66/518.02 ms`。它比 publish-event 50 张好，但仍未低于 TVM p95；top tail 转为 RX capture/wait（例如 image 42 capture `399.88 ms`）和 worker wait `170-180 ms`。因此低同步早退仍只作为诊断开关，不推广到 Cockpit 默认。
