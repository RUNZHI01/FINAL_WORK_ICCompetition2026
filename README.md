...
- 真机 IQ 直传前可先生成板端同步包：`docker run --rm -v "$PWD:/workspace" -e REPO_ROOT=/workspace -e OUT_TAR=/workspace/artifacts/iq_board_sync.tar.gz -e OUT_MANIFEST=/workspace/artifacts/iq_board_sync_manifest.txt iccomp-ubuntu-minimal bash /workspace/scripts/prepare_iq_board_sync.sh`。该脚本只打包，不保存密码；manifest 会提示运行时输入 `SSHPASS` 后再 scp/ssh 到板端。
