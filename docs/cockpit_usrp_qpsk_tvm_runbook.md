...
- IQ direct remains experimental, not the default. The current live OTA profile is `ANALOG_SPS=16`, `ANALOG_AMPLITUDE=24000`: `cockpit_iq_sps16_amp24000_retry_20260709_064549` passed strict decode with sync metric `0.981`, estimated SNR `14.35 dB`, latent MSE `17337`, detected airtime `65.152 ms`, and decode wall time `1.42 s`. A full cockpit/backend IQ-direct + TVM run, `batch-1783552045-1`, also completed 1/1 with fallback 0 and TVM inference `263.687 ms`; its manifest confirmed `sps=16` and `sc16_amplitude=24000`. That live capture had weaker IQ quality (sync `0.866`, estimated SNR `-3.70 dB`, latent MSE `1.10e6`) and transport wall time about `55 s`, so keep QPSK as the reliable default until IQ quality and decode overhead are fixed.
